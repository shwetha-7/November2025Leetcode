...
class Solution:
    def findXSum(self, nums: list[int], k: int, x: int) -> list[int]:
        ...
        for i in range(n-k+1):
            res=self.xsum(nums[i:i+k],x)
            ans.append(res)
        return ans
        
    def xsum(self,nums:list[int],x:int)->int:
        sub_array=list(Counter(nums).items())
        sub_array.sort(key=lambda x:x[0])
        sub_array.sort(key=lambda x:x[1])
        if len(sub_array)<x:
            sum_total=0
            for arr in sub_array:
                sum_total+=arr[0]*arr[1]
            return sum_total
        sum_total=0
        while x:
            curr=sub_array.pop()
            sum_total+=curr[0]*curr[1]
            x-=1
        return sum_total
class TestApp:
    ...
    def test_case_three(self):
        assert Solution().findXSum([9,2,2],3,3)==[13]
